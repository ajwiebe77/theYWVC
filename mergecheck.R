...
mergecheck <- function(temp_pts, temp_pts2, toldist, indxskip, threshAng){
  ...
  if(dist1 < toldist && dist1 <= dist2 && dist1 <= dist3 && dist1 <= dist4){ # merge tip to tip
  ...
  }else if(dist2 < toldist && dist2 <= dist1 && dist2 <= dist3 && dist2 <= dist4){ # merge tip to tail
  ...
  }else if( dist3 < toldist && dist3 <= dist1 && dist3 <= dist2 && dist3 <= dist4){ # merge tail to tip
    i1 <- nrow(temp_pts)
    i2 <- indxskip + 1 # 27 Jan 2023
  ...
  }else if(dist4 < toldist && dist4 <= dist1 && dist4 <= dist2 && dist4 <= dist3){ # merge tail to tail
    i1 <- nrow(temp_pts)
    i2 <- nrow(temp_pts2)
